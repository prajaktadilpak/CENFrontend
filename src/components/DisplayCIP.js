...
import DownloadBag from './downloadBag';
function DisplayCIP({ ports, selectedPort1, setSelectedPort1, selectedPortId, setSelectedPortId }) {
  const [isDownloadBagClicked, setDownloadBagClicked] = useState(false)
  let cipsData = [{
    "id": 1,
    "name": "Ottawa_CIP1",
    "status": "online",
  ...
  const getBag = () => {
    setDownloadBagClicked(true)
  }

  const handleClose= () =>{
    setDownloadBagClicked(false)
  }
  useEffect(() => {
  ...
  return (
    <div className="display-container">
              ...
              <table border="1" style={{ borderCollapse: 'collapse', width: '100%' }}>
                ...
                {ports?.length !== 0 && cipsData?.map((item) => (
                    <tr key={item.id} style={{ backgroundColor: "white" }}>
                      ...
                      <td><button type="submit" className="btn-submit" onClick={getBag}>Bag List</button></td>
      ...
      {isDownloadBagClicked ? (
        <CommonDialog open={isDownloadBagClicked} maxWidth={300} handleClose={handleClose} appSelector={<DownloadBag />} />
      ) : ''}
    </div>
  );
}
